...
import os
import joblib
from autoint import AutoIntModel, predict_model, AutoIntPlus
...
def load_data():
    ...
    field_dims = np.load(f'{data_path}/field_dims_mlp_2nd.npy')
    label_encoders = joblib.load(f'{data_path}/autoIntMLP_label_encoders_2nd.pkl')
    dropout= 0.5
    embed_dim= 16
    ...
    # AutoIntPlus 모델 인스턴스 생성
    model = AutoIntPlus(field_dims, embedding_size=16, att_layer_num=3, att_head_num=2, att_res=True, mlp_units=[128, 64], dropout=0.5)
    
    # 모델 가중치 불러오기
    model([[0 for _ in range(len(field_dims))]])
    model.load_weights(f'{model_path}/autoIntMLP_model_weights_2nd.h5')
