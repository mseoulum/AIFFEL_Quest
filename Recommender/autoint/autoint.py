...
class AutoIntPlus(Model):
    def __init__(self, field_dims, embedding_size, att_layer_num=3, att_head_num=2, 
                 att_res=True, mlp_units=[128, 64], dropout=0.4, embed_dim=16, init_std=0.0001):
        super(AutoIntPlus, self).__init__()
        
        # 기존 AutoInt에서 사용했던 임베딩 레이어
        self.embedding = FeaturesEmbedding(field_dims, embedding_size)
        
        # 멀티헤드 어텐션 레이어 (AutoInt에서 사용했던 부분)
        self.int_layers = [MultiHeadSelfAttention(att_embedding_size=embedding_size, head_num=att_head_num, use_res=att_res) for _ in range(att_layer_num)]
        
        # MLP 레이어 추가
        self.mlp = MultiLayerPerceptron(embedding_size * len(field_dims), hidden_units=mlp_units, dropout_rate=dropout, output_layer=False)
        
        # 최종 출력 레이어
        self.final_layer = Dense(1, activation='sigmoid', kernel_initializer=tf.random_normal_initializer(stddev=init_std))

    def call(self, inputs):
        # 임베딩을 수행하고 상호작용 레이어를 거친 결과를 flatten합니다.
        att_input = self.embedding(inputs)
        ...
        # MLP를 통해 비선형성 추가
        mlp_output = self.mlp(att_output)

        # 최종 예측
        final_output = self.final_layer(mlp_output)
